...
def load_and_preprocess_all_pairs(directory, num_pairs_per_pair):
    all_X = []
    all_y = []

    # Iterate through all files in directory and get pairs
    filenames = os.listdir(directory)
    for filename in filenames:
        if filename.endswith('.obj'):
            base_name = os.path.splitext(filename)[0]
            obj_path = os.path.join(directory, filename)
            pcd_path = os.path.join(directory, base_name + '.pcd')

            if os.path.exists(pcd_path):  # Check if the corresponding PCD exists
                X, y = load_and_preprocess_data(pcd_path, obj_path, num_pairs_per_pair)
                all_X.append(X)
                all_y.append(y)
